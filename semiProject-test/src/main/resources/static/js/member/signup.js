...
const checkAuthKeyBtn = document.querySelector("#checkAuthKeyBtn");   // 인증번호 입력 후 확인 버튼
...
memberEmail.addEventListener("input", e => {
  ...
  fetch("/member/checkEmail?memberEmail=" + inputEmail)
    .then(resp => resp.text())
    .then(count => {        // 1(중복) or 0(중복X)
      // == : 값이 같은 지 ex) "1" == 1 -> true
      // === : 타입까지 같은 지 ex) "1" === 1 -> false
      if (count == 1) {     // memberEmail 이 중복되는 경우
        emailMessage.innerText = "이미 사용중인 이메일 입니다.";
        emailMessage.classList.add("error");
        emailMessage.classList.remove("confirm");
        checkObj.memberEmail = false;   // 유효하지 않은 이메일(중복)
        return;
      }
