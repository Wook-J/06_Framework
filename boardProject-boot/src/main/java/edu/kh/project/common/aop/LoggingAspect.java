...
public class LoggingAspect {
	...
	public void beforeController(JoinPoint jp) {
		...
		String className= jp.getTarget().getClass().getSimpleName();	// AOP 적용된 클래스 이름 얻어오기
		String methodName = jp.getSignature().getName();	// 실행된 컨트롤러 메서드 이름 얻어오기
	...
	public Object aroundServiceImpl(ProceedingJoinPoint pjp) throws Throwable {
		...
		/* ********** @Before 부분 ********** */
		String className= pjp.getTarget().getClass().getSimpleName();	// AOP 적용된 클래스 이름 얻어오기
		String methodName = pjp.getSignature().getName();	// 실행된 컨트롤러 메서드 이름 얻어오기
	...
	public void transactionRollback(JoinPoint jp, Throwable ex) {
		// 해당 메서드에 @Transactional 이 있어야하고, 클래스 상단에 붙어 있으면 수행 안됨
