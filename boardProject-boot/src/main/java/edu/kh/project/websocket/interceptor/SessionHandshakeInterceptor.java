...
public class SessionHandshakeInterceptor implements HandshakeInterceptor{
	...
	public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response, WebSocketHandler wsHandler,
		...
		// - HandShakeInterceptor가 Handler에게 데이터를 전달하는 역할
		
		// request 가 참조하는 객체가 ServletServerHttpReqeust 로 다운캐스팅이 가능한가?
		if(request instanceof ServletServerHttpRequest) {
			...
			// 가로챈 세션을 Handler 에 전달할 수 있게 값 세팅
			attributes.put("session", session);
		}
		
		return true;	// 가로채기 진행여부 (true로 작성해야 세션을 가로채서 Handler에 전달 가능)
	}

	@Override	// 핸들러 동작 후에 수행되는 메서드
