...
public class TestWebSocketHandler extends TextWebSocketHandler{
	...
	// - SessionHandshakeInterceptor 가 가로챈 연결한 클라이언트의 HTTPSession 을 갖고 있음
	//   (attributes 에 추가한 값)
	
	// Collections.synchronizedSet : 동기화된 Set 생성
	// - 여러가지 스레드가 동작하는 환경에서 하나의 컬렉션에 여러 스레드가 접근하여
	//   의도치않은 문제가 발생되지 않게 하기 위해 동기화를 진행하여
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// messages.getPayload() : 통신 시 탑재된 데이터 자체만 꺼내오기
