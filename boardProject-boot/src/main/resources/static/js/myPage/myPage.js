...
if (updateInfo != null) {
  ...
  updateInfo.addEventListener("submit", async e => {
    ...
    if (memberNickname.value.trim().length === 0) {
      alert("닉네임을 입력해주세요");
      // e.preventDefault(); // 제출 막기 (맨앞에 달았음!)
      return;
    }
    ...
    if(currentNickname !== memberNickname.value){
      ...
      // 비동기 요청 (fetch() API 이용)
      // async / await 사용
      // async : 비동기 함수를 만들 때 사용하는 키워드 ("이 함수 내 오래 걸리는 작업 있음..!")
      // await : 비동기 작업의 결과를 기다릴 때 사용 -> 반드시 async 함수 내 에서만 사용가능!!!
      //      -> "이 작업이 끝날 때까지 기다려 주세요!!"
      // 함수 앞부분에 async 키워드를 넣어야함!!! (여기선 익명함수라 e 앞에 붙임!)
      // fetch() 함수 앞에 await 작성!
      const resp = await fetch("/member/checkNickname?memberNickname=" + memberNickname.value);
      const count = await resp.text();

      if(count == 1){
        alert("이미 사용중인 닉네임입니다!");
        // e.preventDefault();    // 비동기 요청 있어서 제갈길 감
        return;
      }
...
// 절대경로로 기본 이미지 URL 설정(얻어오기)
// window.location.origin == http://localhost

let statusCheck = -1;   // -1 : 초기 상태, 0 : 이미지를 삭제, 1 : 새 이미지 선택 (flag 용)
let previousImage = profileImg.src;    // 이전 이미지의 URL 기록 (초기 상태의 이미지 URL 저장)
let previousFile = null;    // 이전에 선택된 파일 객체를 저장

// 이미지 선택 시 미리보기 및 파일 크기 검사
imageInput.addEventListener("change", () => {
  // change 이벤트 : 기존에 있던 값과 달라지는 경우
  ...
  if(file) {    // 파일이 선택된 경우
    if(file.size <= MAX_SIZE){        // 파일 크기가 허용범위 이내인 경우
      const newImageUrl = URL.createObjectURL(file);  // 임시 URL 생성(미리보기 이미지 url 용도)
      profileImg.src = newImageUrl;   // 미리보기 이미지 설정(img 태그의 src에 선택한 파일 임시경로)
      statusCheck = 1;                // 1. 새 이미지 선택 상태 기록
      previousImage = newImageUrl;    // 현재 선택된 이미지를 이전 이미지로 저장(바뀔 일에 대비)
      previousFile = file;            // 현재 선택된 파일 객체를 이전 파일로 저장(바뀔 일에 대비)

    } else {                          // 파일 크기가 허용범위를 초과한 경우
      alert("5MB 이하의 이미지를 선택해주세요!");
      imageInput.value = '';          // 파일 선택 초기화
      // (alert 띄웠지만 이미 선택된 큰 사이즈 파일을 비우는 건 별도임)
      // == imageInput.files = null;
      profileImg.src = previousImage; // 2. 이전 미리보기 이미지로 복원
      
      // 3. 파일 입력 복구 : 이전파일이 존재하면 다시 할당
      if(previousFile){
        const dataTransfer = new DataTransfer();
        // DataTransfer : JS로 파일을 조작할 때 사용되는 인터페이스
        ...
        // --> input 태그의 files 속성은 FileList만 저장가능하므로
        // DataTransfer 를 이용하여 현재 File 객체를 FileList로 변환하여 할당

        dataTransfer.items.add(previousFile);       // 이전 파일(File 객체)을 추가
        imageInput.files = dataTransfer.files;      // 이전 파일로 input 태그의 files 속성 복구
        // DataTransfer에 저장된 파일의 리스트를 FileList 객체로 반환
      }
    }

  } else {      // 파일 선택이 취소된 경우 -> 선택한 파일이 사라짐(value == '')
    profileImg.src = previousImage;   // 이전 미리보기 이미지로 복원

    // 파일입력 복구 : 이전파일이 존재하면 다시 할당
    if(previousFile){
      const dataTransfer = new DataTransfer();
