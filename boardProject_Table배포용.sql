...
--> 0 : 중복 X (해당 이메일 사용중인 회원 없음) -> 이용 가능
--> 1 : 중복 O (해당 이메일 사용중인 회원 있음) -> 이용 불가능
---------------------------------------------------------------------------------

------- UPLOAD_FILE 에서 특정 유저가 올린 파일 확인하기 -------------------------
...
---------------------------------------------
/* 게시글 번호 시퀀스 생성 */
CREATE SEQUENCE SEQ_BOARD_NO NOCACHE;	-- 여기까지 수행(11/14 오전9시14분)
...
SELECT BOARD_NO, BOARD_TITLE, BOARD_CONTENT, BOARD_CODE, READ_COUNT,
	...
	(SELECT COUNT(*) FROM "BOARD_LIKE" WHERE BOARD_NO = 2000) LIKE_COUNT,
	(SELECT IMG_PATH || IMG_RENAME FROM "BOARD_IMG" WHERE BOARD_NO = 2000 AND IMG_ORDER = 0) THUMBNAIL,
	(SELECT COUNT(*) FROM "BOARD_LIKE" WHERE BOARD_NO = 2000) LIKE_CHECK
FROM "BOARD"
JOIN "MEMBER" USING(MEMBER_NO)
WHERE BOARD_DEL_FL = 'N'
AND BOARD_CODE = 1			-- 공지게시판
AND BOARD_NO = 2000;		-- 마지막 게시글
...
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 1',
			  DEFAULT, DEFAULT,	2000, 1, NULL);		-- COMMENT_NO = 2001			1레벨
			 
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 2',
			  DEFAULT, DEFAULT,	2000, 2, NULL);		-- COMMENT_NO = 2002			1레벨
			 
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 3',
			  DEFAULT, DEFAULT,	2000, 3, NULL);		-- COMMENT_NO = 2003			1레벨
			 
-- 부모 댓글 1의 자식 댓글
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식 1',
			  DEFAULT, DEFAULT,	2000, 2, 2001);		-- PARRENT_COMMENT_NO = 2001	2레벨
...
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 2의 자식 1',
			  DEFAULT, DEFAULT,	2000, 1, 2002);		-- PARRENT_COMMENT_NO = 2002	2레벨
			 
-- 부모 댓글 2의 자식 1의 자식 댓글			 
INSERT INTO "COMMENT"	
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 2의 자식 1의 자식!!!',
			  DEFAULT, DEFAULT,	2000, 1, 2008);		-- PARRENT_COMMENT_NO = 2006	3레벨
...
SELECT LEVEL, C.* FROM
	...
	FROM "COMMENT"
	JOIN MEMBER USING(MEMBER_NO)
	WHERE BOARD_NO = 2000) C	-- 인라인 뷰
WHERE COMMENT_DEL_FL = 'N'	--> 삭제되지 않은 댓글이거나
OR 0 != (SELECT COUNT(*) FROM "COMMENT" SUB
				WHERE SUB.PARENT_COMMENT_NO = C.COMMENT_NO
				AND COMMENT_DEL_FL = 'N')
				--> 삭제된 댓글이라도 그 아래에 활성상태인 자식 댓글인 경우
START WITH PARENT_COMMENT_NO IS NULL
CONNECT BY PRIOR COMMENT_NO = PARENT_COMMENT_NO
ORDER SIBLINGS BY COMMENT_NO;
-- SIBLINGS : 같은 레벨 --> 같은 레벨 간 COMMENT_NO 를 오름차순
...
SELECT CHATTING_ROOM_NO
	,(SELECT MESSAGE_CONTENT	-- INLINE 뷰
	FROM (SELECT * FROM MESSAGE M2
		WHERE M2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		ORDER BY MESSAGE_NO DESC) 			-- 가장 최근 메세지번호를 가장먼저 가져오도록 정렬
	WHERE ROWNUM = 1) LAST_MESSAGE		-- 가장 최근 메세지 1개만 선택해서 가져오기
	,TO_CHAR(NVL((SELECT MAX(SEND_TIME) SEND_TIME 
			FROM MESSAGE M
			WHERE R.CHATTING_ROOM_NO  = M.CHATTING_ROOM_NO), CREATE_DATE), 
			'YYYY.MM.DD') SEND_TIME
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2			-- NVL2("값", NULL이 아닌 경우, NULL인 경우)
...
WHERE OPEN_MEMBER = 1
	OR PARTICIPANT = 1
ORDER BY MAX_MESSAGE_NO DESC NULLS LAST;	-- NULLS LAST : 최근 메시지가 없는 경우를 고려
