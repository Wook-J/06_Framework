 ...
 * 
 * - 해당 어노테이션이 작성된 인터페이스는 namespace 에 해당 인터페이스가
 *   작성된 mapper.xml 파일과 연결되어 SQL 호출/수행/결과반환 가능
 * 
 * - MyBaits에서 제공하는 Mapper 상속 객체가 Bean 으로 등록됨
 * 
 * - @Mapper 를 사용할 때 @Repository 는 필요하지 않음!
 *   -> MyBatis가 DAO 객체로 해당 Mapper 인터페이스를 관리하기 때문
 * */
@Mapper
